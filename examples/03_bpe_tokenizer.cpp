...
int main(const int argc, char** argv) {
    try {
        const int firstArg = (argc >= 2 && std::string(argv[1]) == "--bpe") ? 2 : 1;

        if (argc != firstArg + 4) {
            std::cerr << "Usage: " << argv[0] << " [--bpe] path/to/corpus.txt maxLines mergeCount sampleText\n";
            return 1;
        }
        ...
        const tfs::BpeTrainingResult result = trainer.trainFromCorpus(path, maxLines, mergeCount);
